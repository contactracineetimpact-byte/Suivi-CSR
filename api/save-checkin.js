...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!existingCycleId) {
      ...
      let cyclesDeCetteExperience = [];
      if (cyclesIds.length > 0) {
        const formula = 'OR(' + cyclesIds.map((id) => `RECORD_ID()='${id}'`).join(',') + ')';
        const existingCyclesUrl = `https://api.airtable.com/v0/${AIRTABLE_BASE}/${TABLE_CYCLES}?filterByFormula=${encodeURIComponent(formula)}&pageSize=100`;
        const existingCyclesRes = await fetch(existingCyclesUrl, { headers });
        const existingCyclesData = await existingCyclesRes.json();
        cyclesDeCetteExperience = existingCyclesData.records || [];
      }
